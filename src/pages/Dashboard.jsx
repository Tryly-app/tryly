...
export default function Dashboard({ session }) {
  ...
  // --- LÓGICA DE AMIGOS (CORRIGIDA PARA BADGES) ---
  const fetchFriendsData = async () => {
      ...
      if (friendIds.length > 0) {
          ...
          // --- CORREÇÃO AQUI: Buscando 'badge_name' além do 'attribute' ---
          const { data: badges } = await supabase.from('reflections')
            .select('user_id, missions(attribute, badge_name)') // <--- O SEGREDO ESTÁ AQUI
          ...
          const combined = profiles.map(p => {
              ...
              // Mapeia: Se tiver badge_name usa ele, senão usa o attribute (fallback)
  ...
  const submitReflection = async () => {
    ...
    // Passando o badge_name para a IA
