...
import { User, Flame, Clock, Settings, X, Bell, Download, Share, UserPlus, Search, Check, Crown, Rocket } from 'lucide-react';
...
export default function Dashboard({ session }) {
  ...
  const firstName = session.user.user_metadata.full_name?.split(' ')[0] || 'Fundador';
  ...
  const advanceToNextTrail = async () => {
      ...
      if (nextTrail) {
          ...
          alert(`Nova fase iniciada: ${nextTrail.title}`);
          fetchData();
      } else {
          alert("Ciclo completo. Aguarde expansão do sistema.");
      }
  };

  // --- CORREÇÃO AQUI: Buscando e Acumulando Badges ---
  const fetchFriendsData = async () => {
      ...
      if (friendIds.length > 0) {
          ...
          // CORREÇÃO: Adicionei badge_name na query
          const { data: badges } = await supabase.from('reflections')
            .select('user_id, missions(attribute, badge_name)')
            .in('user_id', friendIds);
          
          const combined = profiles.map(p => {
              // Filtra reflexões deste amigo
              const userReflections = badges.filter(b => b.user_id === p.id && b.missions);
              
              // Mapeia: Se tiver badge_name usa ele, senão usa o attribute. Remove duplicados.
              const userBadges = userReflections.map(b => b.missions.badge_name || b.missions.attribute);
              const uniqueBadges = [...new Set(userBadges)]; // Acumula sem repetir o mesmo nome

              return { ...p, badges: uniqueBadges };
          });
  ...
  const searchUser = async () => {
      ...
      if (profile) { setSearchResult(profile); setFriendMsg(''); } else { setSearchResult(null); setFriendMsg("ID não localizado."); }
  };
  ...
  const sendFriendRequest = async () => {
      ...
      if (existing) { setFriendMsg("Conexão já existente ou pendente."); return; }
      await supabase.from('friendships').insert({ user_id: session.user.id, friend_id: searchResult.id });
      setFriendMsg("Solicitação enviada."); setSearchResult(null); setSearchId('');
  };
  ...
  const submitReflection = async () => {
    if (reflectionText.length < 5) return alert("Relato insuficiente. Aprofunde.");
    setLoading(true);
    
    // Passando o badge_name para a nova IA "Mestre"
    const feedback = await processReflection(reflectionText, currentMission.attribute, currentMission.badge_name);
  ...
  const checkReminder = (prog) => {
    ...
    if (!isCompletedToday && prog.status !== 'in_progress') { setReminderMessage({ title: "Eficiência Comprometida", text: "Missão pendente. A inércia gera prejuízo.", action: "Executar Agora" }); setShowReminder(true); } 
    else if (prog.status === 'in_progress') { setReminderMessage({ title: "Processo Incompleto", text: "Finalize o que começou para computar o progresso.", action: "Finalizar" }); setShowReminder(true); }
  };
  ...
  if (loading && !aiResponse) return <div className="container center"><p>Carregando dados...</p></div>;
  ...
  if (view === 'trail_finished') {
      return (
        <div className="container center" style={{justifyContent: 'center'}}>
           ...
           <h2>Ciclo Finalizado</h2>
           <p>Módulo concluído: <strong>{activeTrail.title}</strong></p>
           <button onClick={advanceToNextTrail} style={{marginTop: 30}}>Próximo Nível</button>
        </div>
      );
  }

  if (view === 'reflection') return ( <div className="container"><h2>Relatório de Execução</h2><p className="mb-4">Análise da ação: <strong>"{currentMission?.action_text}"</strong></p><textarea rows="6" placeholder="Descreva os dados da execução e resultados..." value={reflectionText} onChange={e => setReflectionText(e.target.value)} /><button onClick={submitReflection} disabled={loading}>{loading ? 'Processando...' : 'Enviar para Análise'}</button><button className="outline mt-4" onClick={() => setView('dashboard')}>Cancelar</button></div> );
  if (view === 'feedback') return ( <div className="container center" style={{justifyContent:'center'}}><div style={{fontSize: '4rem', marginBottom: 10}}>⚡</div><h2 style={{color: '#7C3AED'}}>Status: Aprovado</h2><div className="mission-card" style={{border: 'none', background: '#F3E8FF', boxShadow: 'none'}}><small style={{fontWeight: 'bold', textTransform: 'uppercase'}}>Análise do Mestre</small><p style={{color: '#4B5563', fontStyle: 'italic', marginTop: 15}}>"{aiResponse}"</p><div className="status-badge" style={{marginTop: 20, background: '#fff'}}>{currentMission?.badge_name ? `🎖️ ${currentMission.badge_name}` : `+ ${currentMission?.attribute}`}</div></div><button onClick={backToHome} style={{marginTop: 30}}>Voltar ao Painel</button></div> );
  if (view === 'mission') return ( <div className="container"><div className="status-badge">Dia {currentMission?.day_number}</div><h1>{currentMission?.title}</h1><p style={{fontSize: '1.1rem', marginTop: 20}}>{currentMission?.description}</p><div className="mission-card"><h3 style={{color: '#7C3AED'}}>Diretriz de Ação</h3><p style={{fontWeight: '600', fontSize: '1.2rem'}}>{currentMission?.action_text}</p></div><div style={{marginTop: 'auto'}}><button onClick={startMission}>Iniciar Protocolo</button><button className="outline mt-4" onClick={() => setView('dashboard')}>Voltar</button></div></div> );
  ...
  return (
    <div className="container" style={{position: 'relative'}}>
      ...
      <header style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20}}>
        <div><small style={{textTransform: 'uppercase', fontWeight: 'bold', color: '#94a3b8', fontSize: '0.7rem'}}>Operador</small><h2 style={{margin: 0}}>{firstName}</h2></div>
      ...
      {!isPro && (
          <div onClick={handleBuyPro} style={{
              ...
              <div style={{display: 'flex', alignItems: 'center', gap: 10}}>
                  ...
                  <div>
                      <strong style={{display: 'block', fontSize: '0.95rem'}}>Acesso Founder</strong>
                      <span style={{fontSize: '0.75rem', opacity: 0.9}}>Ferramentas de alta performance.</span>
                  </div>
              </div>
              <span style={{fontWeight: 'bold', background: '#fff', color: '#7C3AED', padding: '4px 10px', borderRadius: 20, fontSize: '0.8rem'}}>UPGRADE</span>
          </div>
      )}
      ...
      <div style={{flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center'}}>
        {!currentMission ? (<div className="mission-card"><h3>Trilha Finalizada</h3><p>Aguardando input para próximo ciclo.</p><button onClick={advanceToNextTrail} style={{marginTop: 15}}>Avançar</button></div>) : (
            <div className={`mission-card ${locked ? 'locked-card' : ''}`}>
            {locked ? (<> <Clock size={40} style={{color: '#94a3b8', marginBottom: 15}} /> <h3>Objetivo Diário Cumprido</h3> <p>Recarregue para o próximo ciclo.</p> <div style={{background: '#e2e8f0', padding: '8px', borderRadius: 8, display: 'inline-block', fontSize: '0.8rem', fontWeight: 'bold', color: '#64748B', marginTop: 10}}> Reset: 00:00 </div> </>) : (
                <> <div className="status-badge"><Flame size={14} style={{marginRight: 5}}/> Dia {currentMission?.day_number}</div> <h2 style={{fontSize: '1.8rem', marginBottom: 10}}>{currentMission?.title}</h2> <p>{currentMission?.action_text}</p> {progress?.status === 'in_progress' ? <button onClick={finishMission} style={{marginTop: 30}}>Reportar Execução</button> : <button onClick={() => setView('mission')} style={{marginTop: 30}}>Analisar Missão</button>} </>
            )}
            </div>
        )}
      </div>
      ...
      {showProPopup && (
        <div className="popup-overlay" style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 300, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
            <div className="mission-card" style={{width: '90%', maxWidth: '350px', textAlign: 'center', padding: '30px 20px'}}>
                ...
                <h3 style={{marginBottom: 10, color: '#1e293b'}}>Feature em Desenvolvimento</h3>
                <p style={{color: '#64748B', marginBottom: 25, lineHeight: '1.5'}}>
                    Estamos implementando o módulo <strong>Founder</strong>. Acesso será liberado em breve para usuários de alta performance.
                </p>
                <button onClick={() => setShowProPopup(false)} style={{width: '100%'}}>Entendido</button>
            </div>
        </div>
      )}

      {/* MODAL RANKING / AMIGOS */}
      {showFriendsModal && (
          <div className="popup-overlay" style={{position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 300, background: 'rgba(0,0,0,0.8)', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
              <div className="mission-card" style={{width: '90%', maxWidth: '400px', maxHeight: '80vh', display: 'flex', flexDirection: 'column', margin: 0, padding: 0, overflow: 'hidden'}}>
                  <div style={{padding: '15px 20px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}><h3 style={{margin: 0}}>Network</h3><button onClick={() => setShowFriendsModal(false)} style={{width: 'auto', padding: 5, background: 'transparent', color: '#64748B', border: 'none'}}><X size={20}/></button></div>
                  ...
                  <div style={{padding: 20, overflowY: 'auto', flex: 1}}>
                      {friendTab === 'ranking' && (
                        <div style={{display: 'flex', flexDirection: 'column', gap: 15}}>
                            {friendsList.length === 0 ? <p style={{textAlign: 'center', color: '#94a3b8'}}>Network vazia.</p> : friendsList.map(friend => (
                                <div key={friend.id} style={{display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid #f1f5f9', paddingBottom: 10}}>
                                    ...
                                    <div style={{flex: 1}}>
                                        <strong style={{fontSize: '0.95rem'}}>{friend.full_name}</strong>
                                        <div style={{display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 4}}>
                                            {/* AGORA EXIBE OS SELOS ACUMULADOS */}
                                            {friend.badges && friend.badges.length > 0 ? friend.badges.map((b, idx) => (
                                                <span key={idx} style={{fontSize: '0.65rem', background: '#FEF3C7', color: '#D97706', padding: '2px 6px', borderRadius: 4, fontWeight: 'bold'}}>{b}</span>
                                            )) : <span style={{fontSize: '0.7rem', color: '#cbd5e1'}}>Sem tração</span>}
                                        </div>
                                    </div>
                                </div>
                            ))}
